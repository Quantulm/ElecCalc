...
class LectureHall(models.Model):
    ...
    light_blackboard_consumption = models.FloatField("Light bulbs: Blackboard - Average consumption per light bulb (kWh/h)")
    ...
    light_stairs_consumption = models.FloatField("Light bulbs: Stairs - Average consumption per light bulb per hour (kWh/h)")
    ...
    beamer_consumption = models.FloatField( "Beamer - Consumption per beamer per hour (kWh/h)")
    ...
    amplifier_consumption = models.FloatField("Amplifier - Consumption per amplifier per hour (kWh/h)")
    ...
    mic_consumption = models.FloatField("Microphone - Consumption per microphone per hour (kWh/h)")

    cam_count = models.PositiveIntegerField("Camera - Number of cameras")
    cam_power = models.FloatField("Camera - Power per camera (W)")
    cam_consumption = models.FloatField("Camera - Consumption per camera per hour (kWh/h)")
...
class StreamingService(models.Model):
    ...
    streaming_access_consumption = models.FloatField("Access - Consumption per hour (kWh/h)")
    streaming_router_power = models.FloatField("Router - Power (W)")
    streaming_router_consumption = models.FloatField("Router - Consumption per hour (kWh/h)" )
...
class VideoOnDemandService(models.Model):
    ...
    vod_data_center_consumption = models.FloatField("Data center - Energy consumption per hour (kWh/h)")
...
class Living_Situation(models.Model):
    ...
    living_consumption = models.FloatField("Energy consumption per light bulb per hour (kWh/h)" )
...
class Electronic_Device(models.Model):
    ...
    device_consumption = models.FloatField("Energy consumption of the device (kWh)")
...
class Transportation(models.Model):
    ...
    transport_dur_file= models.FileField(upload_to="data")

    transport_frequency = models.FloatField("Transport frequency in days per week")
    transport_freq_file= models.FileField(upload_to="data")
